...
class EditWeaponCpnt(initial: Weapon, option: Option[mutable.ListBuffer[EditStatWithName[Weapon]]])(repStat: HtmlRep[IntBaseStat, EditStat]) extends EditStatWithName[Weapon](initial,option)(repStat){
  ...
  override def randomValue: Weapon = new Weapon(RandomName(),AnyRefBaseStat[Float](Actor.randomWeaponVal _).map(_.round))

  override def create(name: String, intBaseStat: IntBaseStat, action: List[Action]): Weapon = new Weapon(name,intBaseStat,initial.action)
}
