...
class Propose[A :IdXmlRep,B <:raw.HTMLElement](list: mutable.ListBuffer[A],ioHtml: IOHtml[B,A]) extends DomCpnt[Div] {
  ...
  def doFilter( filter : A => Boolean): Unit = {
    seleO.innerHTML = ""
    list.filter(filter).foreach(a => seleO.appendChild(a.newHtml))
  }
}
object Propose{
